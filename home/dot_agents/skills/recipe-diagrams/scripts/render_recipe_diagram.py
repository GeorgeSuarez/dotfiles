...
def ascii_recipe_text(value: str) -> str:
    ...
    replaced = "".join(
        ASCII_REPLACEMENTS.get(character, character) for character in value
    )
...
def parse_recipe_diagram(document: Any) -> RecipeDiagram:
    ...
    recipe_yield = require_recipe_string(
        document.get("yield", ""), "yield", allow_empty=True
    )
    ...
    for column_index, column_value in enumerate(columns_value):
        ...
        if not isinstance(actions_value, list):
            raise RecipeDiagramInputError(
                f"columns[{column_index}].actions must be an array"
            )
        ...
        for action_index, action_value in enumerate(actions_value):
            ...
            if (
                ...
                raise RecipeDiagramInputError(
                    f"{action_field}.rows must contain two integers"
                )
            ...
            actions.append(
                RecipeAction(
                    start_row=start_row,
                    end_row=end_row,
                    label=require_recipe_string(
                        action_value.get("label"), f"{action_field}.label"
                    ),
                )
            )
...
def calculate_column_widths(
    total_width: int, process_column_count: int
) -> tuple[int, ...]:
    ...
    minimum_content_width = (
        minimum_ingredient_width + minimum_process_width * process_column_count
    )
...
def build_recipe_layout(
    diagram: RecipeDiagram, total_width: int
) -> RecipeDiagramLayout:
    ...
    ingredient_lines = tuple(
        wrap_recipe_label(ingredient, column_widths[0])
...
def find_row_action(
    actions: tuple[RecipeAction, ...], row_index: int
) -> RecipeAction | None:
...
def render_recipe_diagram(diagram: RecipeDiagram, total_width: int) -> str:
    ...
    for setup_line in diagram.setup:
        for wrapped_setup_line in wrap_recipe_label(setup_line, total_width - 2):
            lines.append(
                "|" + center_cell_text(wrapped_setup_line, total_width - 2) + "|"
            )
        lines.append(render_horizontal_border(widths))
...
def parse_command_line() -> argparse.Namespace:
    ...
    parser.add_argument(
        "input", type=Path, help="JSON input file, or - for standard input"
    )
    parser.add_argument(
        "--width",
        type=int,
