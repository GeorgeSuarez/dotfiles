...
def wrap_for_pixels(
    text: str, pixel_width: int, font_size: int, padding: int
) -> tuple[str, ...]:
...
def build_png_layout(diagram: RecipeDiagram, width: int) -> PngLayout:
    ...
    setup_heights = tuple(
        len(lines) * line_height + 2 * padding for lines in setup_lines
    )
...
def render_svg(diagram: RecipeDiagram, layout: PngLayout) -> str:
    ...
    title_lines = wrap_for_pixels(
        title, layout.table_width, layout.title_font_size, layout.padding
    )
    parts.append(
        svg_text_lines(
            title_lines,
    ...
    for lines, setup_height in zip(
        layout.setup_lines, layout.setup_heights, strict=True
    ):
        parts.append(
...
def rasterize_svg(svg: str, output_path: Path) -> None:
    ...
    if magick is None:
        raise RecipeDiagramInputError(
            "ImageMagick is required; install the `imagemagick` package"
        )
...
def parse_command_line() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Render recipe dependency JSON as a high-resolution PNG"
    )
    parser.add_argument("input", type=Path, help="JSON input file")
    parser.add_argument("output", type=Path, help="PNG output path")
    parser.add_argument(
        "--width", type=int, default=3840, help="PNG width in pixels (default: 3840)"
    )
