...
// Adapted from pi's examples/extensions/github-issue-autocomplete.ts.
...
async function resolveGitHubRepo(pi: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	...
	if (result.code !== 0) return { ok: false, error: "issue-autocomplete: cwd is not a git repository" };
	...
	return { ok: false, error: "issue-autocomplete: cwd is not a GitHub repository" };
}
...
function filterIssues(issues: GitHubIssue[], query: string): AutocompleteItem[] {
	...
	if (/^\d+$/.test(query)) {
		const numericMatches = issues.filter((issue) => String(issue.number).startsWith(query)).slice(0, MAX_SUGGESTIONS).map(formatIssueItem);
...
function createIssueAutocompleteProvider(current: AutocompleteProvider, getIssues: () => Promise<GitHubIssue[] | undefined>): AutocompleteProvider {
	return {
		async getSuggestions(lines, cursorLine, cursorCol, options): Promise<AutocompleteSuggestions | null> {
			...
			if (options.signal.aborted || !issues || issues.length === 0) return current.getSuggestions(lines, cursorLine, cursorCol, options);
...
export default function (pi: ExtensionAPI): void {
	pi.on("session_start", async (_event, ctx) => {
		...
		if (!resolvedRepo.ok) return; // stay silent outside GitHub checkouts
		...
		const getIssues = async (): Promise<GitHubIssue[] | undefined> => {
			issuesPromise ||= (async () => {
				...
				if (result.code !== 0) {
					if (!loadErrorShown) {
						loadErrorShown = true;
						ctx.ui.notify(`issue-autocomplete: failed to load issues: ${result.stderr.trim() || `exit code ${result.code}`}`, "error");
					}
					return undefined;
				}
				try {
					...
					if (!loadErrorShown) {
						loadErrorShown = true;
						ctx.ui.notify("issue-autocomplete: failed to parse gh issue list output", "error");
					}
					return undefined;
				}
			})();
