...
function findCompactionModel(ctx: ExtensionContext) {
	const value = process.env.PI_COMPACTION_MODEL ?? "opencode-go/ox-alpha-free";
	...
	pi.on("session_before_compact", async (event, ctx) => {
		...
		if (!model) {
			if (!modelWarned) {
				modelWarned = true;
				ctx.ui.notify(
					`cheap-compaction: model not found (PI_COMPACTION_MODEL=${process.env.PI_COMPACTION_MODEL ?? "opencode-go/ox-alpha-free"}); using default compaction`,
					"warning",
				);
			}
			return;
		...
		try {
					...
					messages: [
						{
							role: "user",
							timestamp: Date.now(),
							content: [{ type: "text", text: `${SUMMARY_PROMPT}${previousContext}\n\n<conversation>\n${conversationText}\n</conversation>` }],
						},
					],
				},
			...
			if (!summary.trim()) {
				if (!signal.aborted) ctx.ui.notify("cheap-compaction: empty summary, using default compaction", "warning");
				return;
			}
			...
			if (!signal.aborted) ctx.ui.notify(`cheap-compaction failed, using default compaction: ${error instanceof Error ? error.message : String(error)}`, "warning");
			return;
		}
	});
}
